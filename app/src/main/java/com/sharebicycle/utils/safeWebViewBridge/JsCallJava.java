package com.sharebicycle.utils.safeWebViewBridge;
...
public class JsCallJava {
	...
	public JsCallJava(String injectedName, Class injectedCls) {
		try {
			...
			ZLog.showPost("init js error:" + e.getMessage());
		}
	}
	...
	private String genJavaMethodSign(Method method) {
		...
		if (len < 1 || argsTypes[0] != WebView.class) {
			ZLog.showPost("method(" + sign
					+ ") must use webview to be first parameter, will be pass");
			return null;
		}
		for (int k = 1; k < len; k++) {
			...
			try {
				...
				for (int k = 0; k < len; k++) {
					...
					if ("string".equals(currType)) {
						sign += "_S";
						values[k + 1] = argsVals.isNull(k) ? null : argsVals
								.getString(k);
					...
					while (numIndex > 0) {
						...
						if (currCls == int.class) {
							...
							values[currIndex] = Long.parseLong(argsVals
									.getString(currIndex - 1));
						} else {
							values[currIndex] = argsVals
									.getDouble(currIndex - 1);
						}
						numIndex /= 10;
					}
				}
