...
/***
 * Description:模糊图片 Company: Zhubaoyi Version：2.0
 * 
 * @title BlurImageView.java
 * @author ZXJ
 * @date 2016-4-22
 ***/
public class BlurImageView {
	 ...
	 */
	@SuppressLint("NewApi")
	public static Bitmap doBlur(Context context, Bitmap sentBitmap, int scale,
								float radius) {
		if (sentBitmap == null)
			return null;
		...
		if (radius <= 6 && VERSION.SDK_INT > 16) {// 经测试，radius大于6后，fastBlur效率更高，并且RenderScript在api11以上使用
			Bitmap bitmap = Bitmap.createScaledBitmap(sentBitmap,
					sentBitmap.getWidth() / scale, sentBitmap.getHeight()
							/ scale, false);// 先缩放图片，增加模糊速度
			...
			return fastBlur(sentBitmap, radius, scale);
		}
	}
	...
	public static Bitmap fastBlur(Bitmap sbitmap, float radiusf, int scale) {
		Bitmap bitmap = Bitmap.createScaledBitmap(sbitmap, sbitmap.getWidth()
				/ scale, sbitmap.getHeight() / scale, false);// 先缩放图片，增加模糊速度
